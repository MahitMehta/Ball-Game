...
public class Player extends Circle {
    ...
    public void update(double deltaTime, double previousTime) {
        ...
        double velDIncrement = this.da * (deltaTime - previousTime);

        if (this.right && (this.velX + velIncrement < this.maxVel))
            this.velX += velIncrement;
        else if (!this.left && this.velX >= 0) {
            if (this.velX - velDIncrement >= 0)
                this.velX -= velDIncrement;
            else
                this.velX = 0;
        }

        if (this.down && (this.velY + velIncrement < this.maxVel))
            this.velY += velIncrement;
        else if (!this.up && this.velY >= 0) {
            if (this.velY - velDIncrement >= 0)
                this.velY -= velDIncrement;
            else
                this.velY = 0;
        }

        if (this.up && (Math.abs(this.velY - velIncrement) < this.maxVel))
            this.velY -= velIncrement;
        ...
        if (this.left && (Math.abs(this.velX - velIncrement) < this.maxVel))
            this.velX -= velIncrement;
